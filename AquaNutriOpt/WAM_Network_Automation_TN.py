...
def WAM_Network_Automation_TN(working_path: str, time_periods: str):
    ...
    data_dir = os.path.join('WAM', 'Inputs', 'Reaches') #Directory where all the WAM's outputs, reach *.csv files, are stored
    ...
    COLUMNS_TO_DROP_TP = ['Ingoing', 'Outgoing', 'Ratio']
    Final_Network_TP_df.drop(columns=COLUMNS_TO_DROP_TP, inplace=True)
    ...
    for i in range(len(merged_df_single_obj_optim_TN)):
        if merged_df_single_obj_optim_TN['Area_acres'].iloc[i] < 1 and \
           merged_df_single_obj_optim_TN['Outgoing'].iloc[i] != '':
    ...
    final_out_file_TN = 'WAM_final_output_single_obj_optim_TN.csv'
    ...
    unique_LUID_TN = [x for x in unique_LUID_TN if x != '']
    ...
    final_out_file_LUID_TN = 'WAM_unique_LUID_optim_TN.csv'
    final_out_file_LUID_TN = os.path.join(Outputs_path, final_out_file_LUID_TN)
    # save the numpy array to a CSV file
    unique_LUID_TN.to_csv(final_out_file_LUID_TN, index=False, header=True)
